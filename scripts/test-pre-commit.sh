...
CONFIG_FILE="scripts/pre-commit-header.config"
...
echo "DEBUG: looking for $CONFIG_FILE in $(pwd)"
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  printf 'DEBUG: SEARCH_FOLDERS (%d): %s\n' "${#SEARCH_FOLDERS[@]}" "${SEARCH_FOLDERS[*]-}"
  printf 'DEBUG: INCLUDE_PATTERNS (%d): %s\n' "${#INCLUDE_PATTERNS[@]}" "${INCLUDE_PATTERNS[*]-}"
  printf 'DEBUG: EXCLUDE_PATTERNS (%d): %s\n' "${#EXCLUDE_PATTERNS[@]}" "${EXCLUDE_PATTERNS[*]-}"
else
...
ensure_header_and_build() {
  local file="$1"
  local component="$2"
  # default_first_release niet meer nodig
